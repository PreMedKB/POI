...
def resolution(race, germline_vcf):
  ...
  vcf_bed_df['Chromosome'] = str(vcf_bed_df['Chromosome'].map(lambda x: re.sub('chr|Chr|CHR', '', x)))
  panno_bed['Chromosome'] = str(panno_bed['Chromosome'].map(lambda x: re.sub('chr|Chr|CHR', '', x)))
  ...
  if filter_bed.empty:
       dic_diplotype = pd.DataFrame()
       dic_rs2gt = pd.DataFrame()
       hla_subtypes = pd.DataFrame()
	  ...
	  vcf_df.loc[:,'#CHROM'] = str(vcf_df['#CHROM'].map(lambda x: re.sub('chr|Chr|CHR', '', x)))
	  vcf_df.loc[:,colnames[1]] = int(vcf_df[colnames[1]])
